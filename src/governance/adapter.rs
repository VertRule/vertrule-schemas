//! Adapter origin identifier and external reference types.
//!
//! [`AdapterOriginId`] discriminates the external surface that produced
//! an event. [`AdapterReference`] carries adapter-native lookup keys for
//! round-tripping.
//!
...
//! # Grammar
//!
//! `AdapterOriginId` values match `[a-z][a-z0-9_]{0,63}`. Common
//! constructors are provided for stable identifiers (`jira`, `lang_chain`,
//! `service_now`, `salesforce`, `slack`, `webhook`); arbitrary identifiers
//! that satisfy the grammar go through [`AdapterOriginId::new`].
//!
//! [`GovernanceScope`]: super::GovernanceScope
...
///
/// Surface-neutral newtype over a validated namespace string.
/// Construction of `AdapterOriginId` goes through [`Self::new`], which
/// enforces the grammar below.
///
